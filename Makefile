...
COMPONENTS ?= "module1"?"module1"?"module1" "module2"?"module2"?"module2"
...
ifneq ($(strip $(COMPONENT)),)
	docker buildx build -t $(IMAGE) -f Dockerfile --build-arg LD_FLAGS="$(LD_FLAGS)" --build-context component=$(COMPONENT) --load .
else
	docker buildx build -t $(IMAGE) -f Dockerfile --build-arg LD_FLAGS="$(LD_FLAGS)" --build-context component=. --load .
endif
